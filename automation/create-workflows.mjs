...
const publishNodes = [
  ...
  // TikTok
  http('TT: iniciar envio', pos(4, 6), { method: 'POST', url: `${TIKTOK}/post/publish/video/init/`, auth: 'tiktok', token: `${JOB}.tiktokToken`, body: '={{ JSON.stringify({ post_info: { title: ($json.text || $json.title).slice(0, 2200), privacy_level: $json.integrations.privacy || "SELF_ONLY", disable_duet: false, disable_comment: false, disable_stitch: false, video_cover_timestamp_ms: 1000, brand_organic_toggle: true, brand_content_toggle: false }, source_info: { source: "FILE_UPLOAD", video_size: $json.bytes, chunk_size: $json.bytes, total_chunk_count: 1 } }) }}', headers: { 'Content-Type': 'application/json; charset=UTF-8' } }),
