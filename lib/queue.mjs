...
export function textFor(c, network) {
  ...
  const base = specific || [c.caption || '', c.hashtags || ''].filter(Boolean).join('\n\n');
  return base.trim();
}
  ...
  for (const c of db.contents) {
    ...
    for (const n of c.channels || []) {
      ...
      jobs.push({
        ...
        filePath: renditionPath ? renditionPath(c) : c.media.rendition, thumbPath: c.media.thumb, fileName: c.media.rendition || c.file,
