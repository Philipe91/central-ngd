...
let tunnel = null;
export function tunnelStatus() { return tunnel ? { url: tunnel.url, startedAt: tunnel.startedAt, simulated: !!tunnel.simulated } : null; }
export function stopTunnel() { if (tunnel?.child) tunnel.child.kill(); tunnel = null; }
export function ensureTunnel(port, logFile) {
  // Modo simulado: NGD_SHARE_PUBLIC_URL aponta direto para o servidor de compartilhamento local, sem cloudflared.
  const fixo = (process.env.NGD_SHARE_PUBLIC_URL || '').replace(/\/$/, '');
  if (fixo) { if (tunnel?.url !== fixo) tunnel = { url: fixo, startedAt: new Date().toISOString(), simulated: true }; return Promise.resolve(fixo); }
